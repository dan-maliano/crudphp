$(document).ready(function(){
    TableNotes=$("#tableNotes").DataTable({
        "columnDefs":[{
            "targets": -1,
            "data":null,
            "defaultContent": "<button type='button' class='btnEdit btn btn-success' data-toggle='modal' data-target='#exampleModal'>Update  <i class='fas fa-pen'></i></button> <button class='btnDelete btn btn-danger' type='submit' name='action' >Delete <i class='far fa-trash-alt'></i></button>"
        }]  
    });
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (option==1) {
                ...
                TableNotes.row(rowtablenote).data([id,name,description]);
            }
